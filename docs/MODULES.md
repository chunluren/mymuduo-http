...
### 1.11 Connector — 客户端连接器

**文件**：`src/net/Connector.h`、`src/net/Connector.cc`

**职责**：发起非阻塞 TCP 连接，是 Acceptor 的客户端镜像。支持连接失败后的指数退避重试。
...
void start();           // 发起连接（可在任意线程调用）
void restart();         // 重新连接（在 loop 线程调用）
void stop();            // 停止连接（可在任意线程调用）
```

**设计要点**：
- 非阻塞 connect：`::connect()` 返回 `EINPROGRESS` → 注册 `EPOLLOUT` → 可写时 `getsockopt(SO_ERROR)` 检查结果
- 重试退避：失败后 `loop_->runAfter(retryDelayMs)` 重试，延迟从 500ms 指数增长到 30s
- 状态机：`kDisconnected → kConnecting → kConnected`

---

### 1.12 TcpClient — TCP 客户端

**文件**：`src/net/TcpClient.h`、`src/net/TcpClient.cc`

**职责**：封装 Connector + TcpConnection，是 TcpServer 的客户端镜像。连接建立后的 TcpConnection 与服务端完全相同。
...
void connect();                       // 发起连接
void disconnect();                    // 优雅关闭
void stop();                          // 停止（不再重连）

TcpConnectionPtr connection() const;  // 获取当前连接（可能为空）
EventLoop* getLoop() const;
const std::string& name() const;

void enableRetry();                   // 启用断开后自动重连
bool retry() const;
...
**设计要点**：
- Connector 连接成功 → `newConnection()` 创建 TcpConnection（同 TcpServer 逻辑）
- 连接断开 → `removeConnection()`，若 `retry_` 为 true 则自动调用 `Connector::restart()`
- `connection()` 方法 mutex 保护，可跨线程安全调用
...
### 2.4 HttpClient — HTTP 客户端

**文件**：`src/http/HttpClient.h`

**职责**：基于 Reactor 架构的 HTTP 客户端，内部使用 TcpClient 维持长连接，支持同步/异步调用。

**响应结构体**：
```cpp
struct HttpClientResponse {
    int statusCode = 0;           // 200, 404, 500 等
    std::string statusMessage;     // "OK", "Not Found" 等
    std::unordered_map<std::string, std::string> headers;
    std::string body;
    bool success = false;
    std::string error;             // 错误信息（超时、连接失败等）
};
```
...
void connect();
void disconnect();
void setHeader(const std::string& key, const std::string& value);  // 默认头
...
std::future<HttpClientResponse> asyncPOST(const std::string& path, const std::string& body,
...
HttpClientResponse request(const std::string& method, const std::string& path,
                           ...
                           int timeoutMs = 5000);
std::future<HttpClientResponse> asyncRequest(const std::string& method, const std::string& path,
                                              const std::string& body = "", const std::string& contentType = "");
```
...
```cpp
EventLoop loop;
HttpClient client(&loop, InetAddress(8080, "127.0.0.1"), "MyClient");
client.connect();
...
auto future = client.asyncPOST("/api/users", R"({"name":"Alice"})");
auto resp2 = future.get();
```

**设计要点**：
- 内部使用 `std::queue<promise>` FIFO 匹配请求/响应（HTTP/1.1 保序）
- 响应解析镜像服务端：`memmem()` 找 `\r\n\r\n`，解析状态行 + 头部 + Content-Length + body
- Keep-Alive 长连接复用，enableRetry 自动重连
...
实现 RFC 6455 WebSocket 协议，含服务端和客户端。

### 3.5 ReactorRpcClient — Reactor 版 JSON-RPC 客户端

**文件**：`src/rpc/RpcClient.h`（与 RpcClient 同文件）

**职责**：基于 TcpClient 的长连接 JSON-RPC 客户端，替代阻塞版 RpcClient。

```cpp
ReactorRpcClient(EventLoop* loop, const InetAddress& serverAddr, const std::string& name);

void connect();
void disconnect();

// 同步调用（带超时）
json call(const std::string& method, const json& params = json(), int timeoutMs = 5000);
...
**设计要点**：
- 内部使用 TcpClient 维持长连接 + enableRetry 自动重连
- 通过 request ID + `std::unordered_map<int, promise>` 匹配异步响应
- `call()` = `asyncCall()` + `future.wait_for(timeout)`
...
### 4.4 WebSocketClient — WebSocket 客户端

**文件**：`src/websocket/WebSocketClient.h`

**职责**：基于 Reactor 架构的 RFC 6455 WebSocket 客户端。支持握手验证、掩码帧、自动重连。
...
**完整公开 API**：
```cpp
WebSocketClient(EventLoop* loop, const InetAddress& serverAddr,
...
```cpp
EventLoop loop;
WebSocketClient client(&loop, InetAddress(9500, "127.0.0.1"), "WsClient", "/chat");
...
client.connect();
client.enableRetry();  // 断线自动重连 + 重新握手
loop.loop();
```

**设计要点**：
- 握手流程：TCP 连接成功 → 发送 HTTP Upgrade → 验证 101 + `Sec-WebSocket-Accept`
- **客户端帧必须掩码**（RFC 6455）：`WebSocketFrameCodec::encode(frame, true)`
- 收到 Ping → 自动回 Pong（掩码）
- 收到 Close → 回 Close + 调用 closeCallback
- enableRetry 后断连自动重新握手
